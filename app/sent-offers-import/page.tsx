...
export default function SentOffersImportPage() {
  ...
  // Which candidate offer (index into r.offers) is used as the cost basis
  // per row - defaults to 0 (the cheapest, since matchInquiryItem's SKU-exact
  // path already returns offers ordered by price ASC).
  ...
  const [saveSummary, setSaveSummary] = useState<{ client: string; quoteId: number; items: number }[] | null>(null);
  ...
  // Cost-in-sell-currency via an EUR bridge, so margin math is right even
  // when the matched offer and the price the client was quoted are in
  // different currencies - same convention as the Sourcing Inquiry page.
  ...
  const handleSave = async () => {
    ...
    response.results.forEach((r, i) => {
      ...
      list.push({ item: r.item, offer });
      byClient.set(r.item.client, list);
    });

    const today = new Date().toISOString().slice(0, 10);
    const summary: { client: string; quoteId: number; items: number }[] = [];
    ...
    try {
      for (const [client, lines] of byClient) {
        ...
        summary.push({ client, quoteId: data.id, items: items.length });
      }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">
            Upload or paste prices already offered to clients by SKU/EAN - each row gets matched against the
            current price book for a cost/RRP snapshot, then saved as a real quote per client in Sales Pipeline.
          </p>
        </div>
      ...
      <div className="rounded-xl border border-gray-200 bg-white p-6 space-y-4">
        <div>
          ...
          <p className="mt-1 text-xs text-gray-500">Needs at minimum: client, a product name or SKU/EAN, and the price sent.</p>
          <input
            type="file"
            accept=".xlsx,.xls,.csv"
        ...
        <div className="border-t border-gray-100 pt-4">
          <label className="block text-sm font-medium text-gray-700">…or paste rows</label>
          <textarea
          ...
          />
          <button onClick={handlePasteParse} className="mt-2 rounded-lg border border-gray-300 px-4 py-2 text-sm font-medium text-gray-700 hover:border-gray-400">
            Parse pasted text
          </button>
        </div>
      </div>
      ...
      {rows.length > 0 && (
        <div className="rounded-xl border border-gray-200 bg-white p-6 space-y-3">
          ...
          <p className="text-xs text-gray-500">Confirm each column's role before matching - auto-detected from the header where possible.</p>
          ...
          {preview && (
            <p className="text-xs text-gray-500">
              {preview.items.length} row(s) ready to match{preview.errors.length > 0 ? `, ${preview.errors.length} skipped` : ""}.
            </p>
          )}
          <button onClick={runMatch} disabled={matching || !preview || preview.items.length === 0} className="rounded-lg bg-gray-900 px-4 py-2 text-sm font-medium text-white hover:bg-gray-700 disabled:opacity-50">
            {matching ? "Matching…" : `Match ${preview?.items.length ?? 0} row(s)`}
          </button>
      ...
      {response && (
        <div className="rounded-xl border border-gray-200 bg-white p-6 space-y-4">
          ...
          <div className="border-t border-gray-100 pt-4">
            <button onClick={handleSave} disabled={saving} className="rounded-lg bg-gray-900 px-4 py-2 text-sm font-medium text-white hover:bg-gray-700 disabled:opacity-50">
